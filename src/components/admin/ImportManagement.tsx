...
const ImportManagement = () => {
  ...
  const handleProductImport = async (product: Product) => {
    try {
      if (!product) {
        toast({
          title: "Error",
    ...
    Papa.parse(csvFile, {
      ...
      complete: async (results) => {
        ...
        const products = data.map(row => ({
          ...
          stock_level: parseInt(row.stock_level || '0', 10) || 0,
          weight: parseFloat(row.weight || '0') || 0,
          requiresWeightInput: row.requiresWeightInput === 'true',
          unit: row.unit || '',
          required: row.required === 'true',
        }));

        // For each product in the array, call handleProductImport
        products.forEach(product => handleProductImport(product as any));
